...
*/!(function () {
  ...
  * that contain the substring input by the user. If the
  * input field is empty, all tasks are visible. If the
  * user input does not match any tasks, no tasks are 
  * shown and the user is notified that there are no
  * matching results.
  */
  function filterList() {             
    ...
    for (let taskId in tasks) {
      ...
      * in the string
      * indexOf is case sensitive, so convert tasks to
      * lowercase to check if all lowercase characters have
    ...
    /*
    * If the user input does not match any of the tasks,
    * there are no matching results, no tasks are shown,
    * and and a notification message is displayed to 
    * that effect.
    */
  ...
  */
  function checkResults(subString) {
    /* 
    * Don't check the last child, since that's the blank
